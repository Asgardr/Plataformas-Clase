...
public class RotatingPlatform : MonoBehaviour {
	...
	void Update ()
	{
		...
		if(m_RotateType == RotateType.ENDLESS)
		{
			JustRotate();
		}
		else if(m_RotateType == RotateType.PERIODIC)
		{
			RotateAndStop();
		}
    }
	...
	float _GetRotationAmount(float rotationSpeed, float currentRotation)
	{
		...
		{
			if (currentRotation + frameRotation > m_LoopLimit)
			{
				frameRotation = m_LoopLimit - currentRotation;
			}
		}
